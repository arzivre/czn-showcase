...
export const savedDatasQueryOptions = (page = 1, pageSize = 10, search = '') =>
  queryOptions({
    queryKey: ['most-favorited', page, pageSize],
    queryFn: () => fetch(`/api/most-favorited/${page}/${pageSize}?search=${search}`)
      .then((res) => res.json() as ReturnType<typeof getPaginatedSavedDataWithBookmarks>)
  })

export const Route = createFileRoute('/most-favorited')({
  ...
  return (
    <main>
      <section className="mx-auto min-h-screen relative px-6 lg:px-8 py-24 sm:py-32">
        <h1 className="mb-4 text-2xl">Most Favorited Saved Data</h1>
        <Input autoFocus
          name="search"
          type="search"
          value={searchTerm}
